...
// POLISHOP JavaScript
// VENDAS CORPORATIVAS : v1.0

$(document).ready(function()
{
	if ($("body").attr("class") == "VendasCorporativas")
	{
		TopicoSelecionado();
...
{	
	...
	if (topico == "" && $("body").attr("class") == "VendasCorporativas") { topico = "VendasCorporativas"; }
	$("a#" + topico).addClass("ON");
	var topicoB = "span." + topico.substring(1);
    var topicoC = topicoB.toString();
	$( topicoC ).css({"display" : "block"});
	$( topicoC ).css({"opacity" : "1"});
	if ($("body").attr("class") == "VendasCorporativas") { window.location.hash = topico; }
};